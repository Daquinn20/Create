...
class TLTEngine:
    ...
    def calculate_mfi(high: pd.Series, low: pd.Series, close: pd.Series,
        ...
        mfr = mf_pos_sum / (mf_neg_sum + 1e-10)  # Avoid div by zero
        mfi = 100 - (100 / (1 + mfr))
        return mfi
    ...
    def calculate_cmf(high: pd.Series, low: pd.Series, close: pd.Series,
        ...
        hl_range = high - low
        hl_range = hl_range.replace(0, 1e-10)  # Avoid div by zero
    ...
    def calculate_mansfield_rs(self, stock_close: pd.Series,
        ...
        """
        Calculate Mansfield Relative Strength (MRS)
        MRS = ((Stock/Benchmark) / SMA(Stock/Benchmark, 52wk) - 1) * 10
        """
        if self.benchmark_data is None or self.benchmark_data.empty:
            return pd.Series(0, index=stock_close.index)

        # Get benchmark close - handle column name variations
    ...
    def _calculate_composite_score(self, lr_ratio, cmf, cmf_rising, mrs, mrs_rising,
        ...
        # LR Ratio contribution (0-30 points)
        score += min(lr_ratio * 20, 30)

        # CMF contribution (0-25 points)
        score += min(cmf * 100, 25)

        # MRS contribution (0-20 points)
        if mrs >= 0:
            score += 20
        elif mrs_rising:
            score += 10

        # MRS rising bonus (10 points)
        if mrs_rising:
            score += 10

        # MA position contribution (0-15 points)
        if above_ma200:
            score += 15
        elif above_ma50:
            score += 10
