...
from tlt_engine_core import TechnicalIndicators, TLTEngine
...
# TechnicalIndicators is imported from tlt_engine_core (single source of truth)
...
# TLTEngine is imported from tlt_engine_core (single source of truth)
...
class StockScreener:
    ...
    def _process_single_vcp(self, symbol: str, info_lookup: Dict, spy_data: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            # 9 Criteria checks (optimized VCP)
            c1_rsi = 45 <= rsi <= 75
            c2_above_200 = current_price > sma_200
            c3_above_50 = current_price > sma_50_current
            c4_within_3_of_10 = pct_from_10sma <= 3  # Within 3% of 10-day SMA
            c5_within_5_of_20 = pct_from_20sma <= 5  # Within 5% of 20-day SMA
            c6_rs_rising = rs_rising
            c7_atr_compress = current_atr_pct <= atr_20th_percentile
            c8_vol_trend = (vol_20d_avg < vol_60d_avg) and (sma_50_slope >= 0)
            c9_near_high = pct_from_52wk_high <= 25  # Within 25% of 52-week high

            criteria_results = [c1_rsi, c2_above_200, c3_above_50, c4_within_3_of_10,
                               c5_within_5_of_20, c6_rs_rising, c7_atr_compress,
                               c8_vol_trend, c9_near_high]
            ...
            return {
                ...
                "ATR Compress": "PASS" if c7_atr_compress else "FAIL",
                ...
                "% from 52wk": round(pct_from_52wk_high, 1),
        ...
        try:
            ...
            for i in range(-cross_lookback, 0):
                if i - 1 >= -len(rsi):
                    ...
                    if prev_rsi <= 50 and curr_rsi_val > 50:
                        c3_rsi_cross = True
                        break
    ...
    def screen_buy_trigger(self, symbols: List[str], stock_info: pd.DataFrame = None,
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            ...
            for future in as_completed(future_to_symbol):
                ...
                if batch_email_callback and completed > 0 and completed % self.BATCH_SIZE == 0:
                    if results:
                        batch_df = pd.DataFrame(results)
                        batch_email_callback(batch_df, f"Buy Trigger - Batch {completed // self.BATCH_SIZE}")
        ...
        if not df_results.empty:
            df_results = df_results.sort_values(
                by=["Grade", "RSI"],
                ascending=[True, False]  # PASS first, then highest RSI first
            )
        return df_results
    ...
    def _process_single_parabolic_pullback(self, symbol: str, info_lookup: Dict) -> Optional[Dict]:
        """Process a single stock for Parabolic Pullback detection
        Finds stocks that were overbought (RSI 70-75) in past 5 days,
        ...
        try:
            ...
            # Past 5 days RSI (excluding today)
            past_5d_rsi = rsi.iloc[-6:-1]
            max_rsi_5d = past_5d_rsi.max() if len(past_5d_rsi) > 0 else 0

            # ATR 5 days ago
            atr_5d_ago = atr.iloc[-6] if len(atr) >= 6 else 0

            # === CRITERIA ===
            # 1. Max RSI in past 5 days was between 70-75 (was overbought)
            c1_was_overbought = 70 <= max_rsi_5d <= 75

            # 2. Current RSI between 65-70 (pulled back to better entry)
            c2_rsi_pullback = 65 <= current_rsi <= 70

            # 3. ATR rising (current > 5 days ago)
            c3_atr_rising = current_atr > atr_5d_ago if atr_5d_ago > 0 else False

            # 4. MACD histogram positive (momentum still positive)
            c4_macd_positive = current_macd_hist > 0

            # 5. Price above 50 SMA (uptrend)
            c5_above_50sma = current_price > sma_50.iloc[-1] if not pd.isna(sma_50.iloc[-1]) else False

            # 6. Price above 200 SMA (long-term uptrend)
            c6_above_200sma = current_price > sma_200.iloc[-1] if not pd.isna(sma_200.iloc[-1]) else False

            # All criteria must pass
            criteria = [c1_was_overbought, c2_rsi_pullback, c3_atr_rising, c4_macd_positive, c5_above_50sma, c6_above_200sma]
            passed_count = sum(criteria)
            is_pullback = all(criteria)
            ...
            return {
                ...
                "Sector": stock_data.get("Sector", ""),
                "Price": round(current_price, 2),
                "Was OB (70-75)": "PASS" if c1_was_overbought else "FAIL",
                "RSI Now 65-70": "PASS" if c2_rsi_pullback else "FAIL",
                "ATR Rising": "PASS" if c3_atr_rising else "FAIL",
                "MACD Positive": "PASS" if c4_macd_positive else "FAIL",
                "Above 50 SMA": "PASS" if c5_above_50sma else "FAIL",
                "Above 200 SMA": "PASS" if c6_above_200sma else "FAIL",
                "Score": f"{passed_count}/6",
                "Grade": "PASS" if is_pullback else "FAIL",
                "RSI Now": round(current_rsi, 1),
                "Max RSI 5d": round(max_rsi_5d, 1),
    ...
    def screen_parabolic_pullback(self, symbols: List[str], stock_info: pd.DataFrame = None,
                                   batch_email_callback=None) -> pd.DataFrame:
        """
        Parabolic Pullback Screen (6 Criteria) - PARALLEL PROCESSING
        Finds stocks that pulled back from overbought to better entry.
        """
        results = []
        progress = st.progress(0)
        status = st.empty()
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            future_to_symbol = {
                executor.submit(self._process_single_parabolic_pullback, symbol, info_lookup): symbol
                for symbol in symbols
            }
            ...
            for future in as_completed(future_to_symbol):
                ...
                if batch_email_callback and completed > 0 and completed % self.BATCH_SIZE == 0:
                    if results:
                        batch_df = pd.DataFrame(results)
                        batch_email_callback(batch_df, f"Parabolic Pullback - Batch {completed // self.BATCH_SIZE}")
    ...
    def _process_single_tlt(self, symbol: str, info_lookup: Dict,
                            tlt_engine: 'TLTEngine') -> Optional[Dict]:
        """Process a single stock for TLT scan - designed for parallel execution"""
        try:
            df = self.fetcher.get_historical_data(symbol, "1y")
            if df is None or len(df) < 60:
                return None

            analysis = tlt_engine.analyze_stock(df)
            if analysis is None:
                return None
            ...
            return {
                ...
                "Signal": f"{analysis['Tier_Emoji']} {analysis['Signal_Tier']}",
                "Score": analysis["Composite_Score"],
                "MFI": analysis["MFI"],
                "RSI": analysis["RSI"],
                "LR Ratio": analysis["LR_Ratio"],
                "CMF": analysis["CMF"],
                "CMF Rising": "Yes" if analysis["CMF_Rising"] else "No",
                "MRS": analysis["MRS"],
                "MRS Rising": "Yes" if analysis["MRS_Rising"] else "No",
                "vs MA200": f"{analysis['vs_MA200']:+.1f}%",
                "vs 52wHigh": f"{analysis['vs_52wHigh']:.1f}%",
                "RelVol": analysis["RelVol"],
                "ATR": analysis["ATR"],
                "StopTight": analysis["StopTight"],
    ...
    def screen_tlt(self, symbols: List[str], stock_info: pd.DataFrame = None,
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            future_to_symbol = {
                executor.submit(self._process_single_tlt, symbol, info_lookup, tlt_engine): symbol
                for symbol in symbols
            }
            ...
            for future in as_completed(future_to_symbol):
                ...
                progress.progress(completed / len(symbols))
                status.text(f"TLT Scan: {symbol} ({completed}/{len(symbols)}) - {len(results)} analyzed")

                if batch_email_callback and completed > 0 and completed % self.BATCH_SIZE == 0:
                    if results:
                        batch_df = pd.DataFrame(results)
                        batch_email_callback(batch_df, f"TLT - Batch {completed // self.BATCH_SIZE}")
        ...
        if not df_results.empty:
            ...
            df_results = df_results.drop(columns=["_tier_order"])
...
# ============================================================================
# SIGNAL HISTORY (Neon/SQLite) — backed by tlt_signal_tracker.py
# ============================================================================

TIER_ORDER = ["OVERSOLD", "SPRING", "SURGE", "LEADER", "DANGER"]
TIER_EMOJI = {
    "OVERSOLD": "🟡", "SPRING": "🌱", "SURGE": "🔵",
    "LEADER": "🚀", "DANGER": "🔴",
}


def _signal_db_connection():
    """Neon if DATABASE_URL set, else local SQLite file written by the tracker."""
    db_url = get_secret("DATABASE_URL") or os.getenv("DATABASE_URL")
    if db_url:
        import psycopg2
        return psycopg2.connect(db_url), "postgres"
    sqlite_path = Path(__file__).parent / "tlt_signals.db"
...
def _load_signal_history() -> pd.DataFrame:
    """Read tlt_signals table. Returns empty df if no DB/table yet."""
    try:
        conn_info = _signal_db_connection()
    except Exception as e:
        st.warning(f"Signal history DB unavailable: {e}")
        return pd.DataFrame()
    if conn_info[0] is None:
        return pd.DataFrame()
    conn, backend = conn_info
    try:
        df = pd.read_sql_query(
            "SELECT ticker, signal_date, tier, mode, universe, signal_price, "
            "mfi, rsi, lr_ratio, cmf, mrs, composite_score, "
            "vs_ma50, vs_ma200, vs_52w_high, rel_vol, "
            "fwd_30d_return, fwd_60d_return, fwd_90d_return, "
            "fwd_30d_captured_at, fwd_60d_captured_at, fwd_90d_captured_at, "
            "created_at "
            "FROM tlt_signals ORDER BY signal_date DESC, ticker",
            conn,
        )
    except Exception as e:
        st.warning(f"Could not read tlt_signals table: {e}")
        df = pd.DataFrame()
    finally:
        conn.close()
    if not df.empty:
...
def _render_signal_history(df: pd.DataFrame) -> None:
    available_universes = sorted(df["universe"].dropna().unique().tolist()) if "universe" in df.columns else ["master"]
    universe_options = ["All"] + available_universes
    universe_choice = st.radio("Universe", universe_options, horizontal=True, key="signal_hist_universe")
    if universe_choice != "All":
        df = df[df["universe"] == universe_choice]
    if df.empty:
        st.info(f"No signals captured yet for universe '{universe_choice}'.")
        return
    ...
    c1.metric("Total signals", f"{total_signals:,}")
    c2.metric("Unique tickers", f"{unique_tickers:,}")
    c3.metric("Latest scan", str(latest_date))
    c4.metric("Date range", date_range)

    st.markdown("---")
    st.subheader("Today's Signals")
    today_df = df[df["signal_date"] == latest_date]
    if today_df.empty:
        st.write("No signals on latest scan date.")
    else:
        tier_counts = today_df["tier"].value_counts().reindex(TIER_ORDER, fill_value=0)
        cols = st.columns(len(TIER_ORDER))
        for col, tier in zip(cols, TIER_ORDER):
            col.metric(f"{TIER_EMOJI[tier]} {tier}", int(tier_counts[tier]))

    st.markdown("---")
    st.subheader("Forward-Return Performance by Tier")
    perf = _aggregate_by_tier(df)
    if perf.empty:
        st.info("Forward returns will appear once signals are 30+ days old.")
    else:
    ...
    with f1:
        sel_tiers = st.multiselect("Tier", TIER_ORDER, default=TIER_ORDER)
    with f2:
        sel_ticker = st.text_input("Ticker filter (optional)").strip().upper()
    with f3:
        min_date, max_date = df["signal_date"].min(), df["signal_date"].max()
        date_filter = st.date_input("Date range", value=(min_date, max_date))

    filt = df[df["tier"].isin(sel_tiers)]
    if sel_ticker:
        filt = filt[filt["ticker"].str.contains(sel_ticker, na=False)]
    if isinstance(date_filter, tuple) and len(date_filter) == 2:
        filt = filt[(filt["signal_date"] >= date_filter[0]) &
                    (filt["signal_date"] <= date_filter[1])]
    st.caption(f"{len(filt):,} rows")
    display_cols = ["signal_date", "ticker", "tier", "mode", "universe", "signal_price",
                    "composite_score", "fwd_30d_return", "fwd_60d_return",
                    "fwd_90d_return", "rsi", "lr_ratio", "cmf", "mrs"]
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"tlt_signals_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        mime="text/csv",
    )


def _aggregate_by_tier(df: pd.DataFrame) -> pd.DataFrame:
    """Per-tier win rate + avg return at each forward window."""
    rows = []
    for tier in TIER_ORDER:
        tier_df = df[df["tier"] == tier]
        if tier_df.empty:
            continue
        row = {"Tier": f"{TIER_EMOJI[tier]} {tier}", "Total signals": len(tier_df)}
        for window in (30, 60, 90):
            col = f"fwd_{window}d_return"
            valid = tier_df[col].dropna()
            row[f"N {window}d"] = int(len(valid))
            if len(valid) > 0:
                row[f"Avg {window}d %"] = round(float(valid.mean()), 2)
                row[f"Win % {window}d"] = round(float((valid > 0).mean() * 100), 1)
                row[f"Median {window}d %"] = round(float(valid.median()), 2)
            else:
                row[f"Avg {window}d %"] = None
                row[f"Win % {window}d"] = None
                row[f"Median {window}d %"] = None
        rows.append(row)
...
def main():
    ...
    page = st.sidebar.radio("Select Module",
                            ["Chart Dashboard", "Stock Screener", "Signal Scanner", "TLT Scanner",
    ...
    # ========================================================================
        ...
        screen_type = st.selectbox(
            "Select Screen Type",
            ["VCP Compression", "Momentum Expansion", "Pullback", "Quinn Favorite", "Short Term Momentum", "Parabolic", "Buy Trigger", "Oversold", "Sell"],
            index=0
        )
        ...
        if screen_type == "VCP Compression":
            ...
            st.subheader("Pre-Parabolic Screen (Optimized)")
            st.success("**Backtest: 67.6% Win Rate | +20.8% Avg 20-Day Return | -15% Stop Loss**")
            criteria_col1, criteria_col2 = st.columns(2)
            with criteria_col1:
                st.markdown("""
                **REQUIRED (must pass all 4):**
                - **RSI 65-70** (optimal entry zone)
                - **Volume >= 1.3x** (surge starting)
                - **ADX > 20** (trend confirmed)
                - **Shallow DD > -15%** (holding gains)
                """)
            with criteria_col2:
                st.markdown("""
                **OPTIONAL (for scoring):**
                - RSI crossed 65 recently
                - MACD rising
                - ATR rising
                - Above 50/200 SMA
    ...
    # TLT SCANNER
    # ========================================================================
    elif page == "TLT Scanner":
        st.header("TLT Scanner - Trend Liquidity Timing")

        st.markdown("""
        The TLT Scanner analyzes stocks using a multi-indicator approach to identify
        ...
        st.write(f"**Stocks to scan:** {len(stocks)} (Index: {selected_index}, Sector: {sector})")
        ...
        with option_col2:
            auto_email = st.checkbox("Auto-email results when complete", value=True, key="tlt_email")

        # Run button
        if st.button("Run TLT Scan", type="primary", key="tlt_run"):
            screener = StockScreener(fetcher)
            ...
            if not results.empty:
                ...
                def color_tlt_signal(val):
                    if "LEADER" in str(val):
                        return "background-color: #90EE90"  # Green
                ...
                if not display_results.empty:
                    ...
                    # Save and download
                    final_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    final_csv_path = checkpoint_dir / f"FINAL_TLT_{final_timestamp}.csv"
                    display_results.to_csv(final_csv_path, index=False)
                    st.info(f"Results saved to: {final_csv_path}")
                    ...
                    st.download_button(
                        "Export to CSV",
                        csv,
                        f"tlt_scan_{datetime.now().strftime('%Y%m%d')}.csv",
                        "text/csv",
                        key="tlt_download"
                    )
                    ...
                    if auto_email:
                        with st.spinner("Sending email..."):
                            try:
                                if send_email_with_csv(display_results, "TLT Scanner"):
                                    st.success(f"Results emailed to {EMAIL_RECIPIENT}")
                            except Exception as e:
                                st.warning(f"Email failed but results saved locally: {e}")
                else:
                    st.warning(f"No stocks matched the filter: {filter_signals}")
            else:
                st.warning("No stocks could be analyzed")
    ...
    # SIGNAL PERFORMANCE HISTORY
    # ========================================================================
    elif page == "Signal Performance History":
        st.header("TLT Signal Performance History")
        st.caption("Daily-captured TLT signals with 30 / 60 / 90-day forward returns. "
                   "Populated by tlt_signal_tracker.py (GitHub Actions, weekdays).")

        df_signals = _load_signal_history()

        if df_signals.empty:
            st.info("No signals captured yet. The daily scan will populate this table.")
        else:
            _render_signal_history(df_signals)
